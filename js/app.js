...
(() => {
  ...
  // Codes-barres appris au fil de l'eau (voir SyncManager.fetchEanCodes /
  // registerEanCode) : complète EAN_TO_REFERENCE (data.js, figé au build)
  // sans jamais devoir reconstruire l'app pour une nouvelle référence.
  // Mis en cache localement pour continuer à fonctionner hors-ligne une
  // fois synchronisé au moins une fois.
  const EanCache = {
    STORAGE_KEY: 'webza_learned_ean_codes',
    _readAll() {
      try {
        const raw = localStorage.getItem(this.STORAGE_KEY);
        return raw ? JSON.parse(raw) : {};
    ...
    lookup(ean) {
      return this._readAll()[ean] || EAN_TO_REFERENCE[ean] || null;
    },
    mergeFromServer(map) {
      const current = this._readAll();
      localStorage.setItem(this.STORAGE_KEY, JSON.stringify({ ...current, ...(map || {}) }));
    },
    remember(ean, reference) {
      const current = this._readAll();
      current[ean] = reference;
      localStorage.setItem(this.STORAGE_KEY, JSON.stringify(current));
    },
    // File d'attente des associations pas encore renvoyées au serveur
    // (créées hors-ligne, ou échec réseau ponctuel) — voir syncPendingEanAssociations,
    // appelée par le même minuteur périodique que la synchro des opérations.
    PENDING_KEY: 'webza_pending_ean_codes',
    _readPending() {
      try {
    ...
    queuePending(ean, reference) {
      const list = this._readPending().filter((p) => p.ean !== ean);
      list.push({ ean, reference });
    ...
    hasPending() {
      return this._readPending().length > 0;
    },
  };
  ...
  function renderServerBadge() {
    // Le badge de statut sur l'accueil a été retiré (le statut de connexion
    // ne s'affiche plus que sur l'écran Profil — voir renderProfileSyncCard).
    // Cette fonction ne fait plus que relayer vers cette carte ; conservée
    // sous ce nom pour ne pas devoir toucher tous ses appelants existants.
  ...
  /* ---- Scan caméra générique (BarcodeDetector natif, pas de lib externe) ----
     Utilisé pour le container ET pour la référence produit (voir EAN_TO_REFERENCE,
     data.js) : openBarcodeScanner(onResult) prend un callback appelé avec la
     valeur brute décodée — chaque appelant décide quoi en faire. */
    ...
    const tick = async () => {
      ...
      try {
        ...
        if (codes && codes.length > 0) {
          ...
          const callback = scannerOnResult;
          closeBarcodeScanner();
  ...
  // Remplit le champ Référence à partir d'une valeur résolue (scan reconnu
  // ou association tout juste créée) — factorisé car appelé depuis deux
  // endroits (scan direct et confirmation de la modale d'association).
  function applyScannedReference(ref) {
    state.formState.reference = ref;
    $('#ref-input').value = ref;
    state.formState.referenceError = null;
    setFieldError('ref-error', null);
    $('#ref-clear-btn').hidden = ref.length === 0;
    renderRefSuggestions(ref);
  }

  /* ---- Association d'un code-barres inconnu à une référence ----
     Voir SyncManager.fetchEanCodes/registerEanCode et EanCache (data.js).
     Le catalogue de référence sert d'auto-complétion (datalist), mais la
     saisie reste libre : un noteur peut associer une référence qui n'y
     figure pas encore. */
  let pendingEanToAssociate = null;
  function openEanAssocModal(ean) {
    pendingEanToAssociate = ean;
    $('#ean-assoc-code-display').textContent = ean;
    $('#ean-assoc-ref-input').value = '';
    $('#ean-assoc-modal-backdrop').hidden = false;
    setTimeout(() => $('#ean-assoc-ref-input').focus(), 50);
  }
  function closeEanAssocModal() {
    $('#ean-assoc-modal-backdrop').hidden = true;
    pendingEanToAssociate = null;
  }
  async function confirmEanAssociation() {
    const ean = pendingEanToAssociate;
    const ref = $('#ean-assoc-ref-input').value.trim().toUpperCase();
    if (!ean) return;
    if (!ref) {
      showToast('Saisissez une référence pour associer ce code.');
      return;
    }
    applyScannedReference(ref);
    EanCache.remember(ean, ref);
    closeEanAssocModal();
    const result = await SyncManager.registerEanCode(ean, ref);
    if (result.success) {
      showToast(`Code associé à ${ref} — les prochains scans le reconnaîtront directement.`);
    } else {
      EanCache.queuePending(ean, ref);
      showToast(`Référence ${ref} utilisée pour cette opération — l'association sera renvoyée au serveur dès la reconnexion.`);
    }
  }
  function skipEanAssociation() {
    const ean = pendingEanToAssociate;
    if (ean) applyScannedReference(ean);
    closeEanAssocModal();
    showToast('Code-barres utilisé tel quel — vérifiez la référence.');
  }
  ...
  function renderForm() {
    ...
    // Truck / grade / defect conditional blocks
    ...
    $('#defect-block').hidden = fs.operationType !== 'reception';
    $('#defect-toggle').setAttribute('aria-pressed', fs.hasDefect ? 'true' : 'false');
  ...
  function validateAndSubmitOperation() {
    ...
    const operation = {
      ...
      timestamp: Date.now(),
      syncStatus: 'PENDING',
      hasDefect: fs.operationType === 'reception' ? !!fs.hasDefect : false,
  ...
  function openEditModal(op) {
    ...
    $('#edit-defect-block').hidden = opType !== 'reception';
    if (opType === 'reception') {
      $('#edit-defect-toggle').setAttribute('aria-pressed', op.hasDefect ? 'true' : 'false');
    }
    if (opType === 'sortie') {
  ...
  async function saveEditModal() {
    ...
    const updated = {
      ...
      truckType,
      grade,
      hasDefect: opType === 'reception' ? ($('#edit-defect-toggle')?.getAttribute('aria-pressed') === 'true') : editingOp.hasDefect,
    };
  ...
  function exportToExcel() {
    ...
    const header = '\uFEFFID;Date Heure;Type Opération;Référence Produit;Emplacement;Quantité;Opérateur;Expédition / Grade;Défaut signalé;Statut Sync\n';
    const body = ops
      .map((op) => {
        ...
        const defect = op.hasDefect ? 'Oui' : 'Non';
        return `${op.id};${dateStr};${op.type.toUpperCase()};${op.reference};${op.location};${op.quantity};${op.operatorName};${detail};${defect};${op.syncStatus}`;
      })
      .join('\n');
  ...
  function renderProfileSyncCard() {
    const icon = $('#profile-sync-icon');
    const title = $('#profile-sync-title');
    const indicator = $('#profile-sync-indicator');
    if (!icon) return;
    if (!WebzaConfig.isServerEnabled()) {
      icon.setAttribute('fill', '#94A3B8');
      title.textContent = 'Connexion au serveur désactivée';
      title.style.color = '#94A3B8';
      indicator.style.background = '#94A3B8';
    } else if (state.isServerConnected === true) {
      icon.setAttribute('fill', '#10B981');
      title.textContent = 'Système synchronisé';
      title.style.color = 'var(--success)';
      indicator.style.background = 'var(--success)';
    } else {
      icon.setAttribute('fill', '#FBBF24');
      title.textContent = 'Serveur non joignable';
  ...
  async function testServerConnection(customUrl) {
    const wasEnabled = WebzaConfig.isServerEnabled();
    ...
    if (res.success) {
      ...
      refreshEanCatalog();
      flushPendingEanAssociations();
    } else {
      state.isServerConnected = false;
      state.lastSyncMessage = res.message;
      if (wasEnabled && !res.disabled) {
        // Échec réel pendant que la connexion était activée (pas le cas où
        // elle était déjà coupée manuellement) : on la désactive nous-même,
        // sans que le noteur ait besoin de rouvrir Réglages pour le faire.
        WebzaConfig.setServerEnabled(false);
        renderServerEnabledToggle();
        showToast('Connexion au serveur perdue — désactivée automatiquement.');
      } else {
        showToast(res.message);
      }
    }
  ...
  // Récupère le catalogue EAN complet côté serveur (associations faites
  // depuis N'IMPORTE QUEL téléphone) et le fusionne dans le cache local —
  // appelé à chaque connexion confirmée, pas en boucle serrée.
  async function refreshEanCatalog() {
    const map = await SyncManager.fetchEanCodes();
    if (map && Object.keys(map).length > 0) EanCache.mergeFromServer(map);
  }

  // Renvoie les associations créées hors-ligne (ou suite à un échec réseau
  // ponctuel) dès que la connexion est confirmée — voir EanCache.queuePending,
  // appelée par confirmEanAssociation et par le minuteur périodique.
  async function flushPendingEanAssociations() {
    const pending = EanCache._readPending();
    for (const { ean, reference } of pending) {
      const result = await SyncManager.registerEanCode(ean, reference);
      if (result.success) EanCache.dequeuePending(ean);
    }
  }
  ...
  function renderServerEnabledToggle() {
    const enabled = WebzaConfig.isServerEnabled();
    const btn = $('#server-enabled-toggle');
    btn.setAttribute('aria-checked', enabled ? 'true' : 'false');
    $('#server-enabled-hint').textContent = enabled ? 'Activée' : 'Désactivée — aucune tentative de connexion';
  }
  ...
  // Clic sur le badge d'état serveur : simple nouvelle tentative de
  // connexion (l'adresse elle-même n'est plus jamais éditable ni affichée
  // depuis l'interface — voir config.js).
  ...
  function wireEvents() {
    ...
    $('#ref-scan-btn').addEventListener('click', () => openBarcodeScanner((value) => {
      const known = EanCache.lookup(value);
      if (known) {
        applyScannedReference(known);
        showToast(`Référence ${known} reconnue.`);
      } else {
        openEanAssocModal(value);
      }
    }));
    ...
    $('#defect-toggle').addEventListener('click', () => {
      state.formState.hasDefect = !state.formState.hasDefect;
      renderForm();
    });
    ...
    $('#edit-defect-toggle').addEventListener('click', () => {
      const pressed = $('#edit-defect-toggle').getAttribute('aria-pressed') === 'true';
      $('#edit-defect-toggle').setAttribute('aria-pressed', pressed ? 'false' : 'true');
    });
    ...
    $('#container-scan-btn').addEventListener('click', () => openBarcodeScanner((value) => {
      // Pas de validation automatique après le scan (voir openBarcodeScanner) :
      // un mauvais code peut être lu par erreur — on remplit le champ et
      // l'opérateur doit relire puis valider lui-même via "Prendre en charge".
      $('#container-code-input').value = value;
      setFieldError('container-code-error', null);
      showToast(`Code scanné : ${value} — vérifiez puis validez.`);
    }));
    $('#scanner-close-btn').addEventListener('click', closeBarcodeScanner);

    // --- Association code-barres inconnu ---
    $('#ean-assoc-close-btn').addEventListener('click', skipEanAssociation);
    $('#ean-assoc-skip-btn').addEventListener('click', skipEanAssociation);
    $('#ean-assoc-confirm-btn').addEventListener('click', confirmEanAssociation);
    $('#ean-assoc-ref-input').addEventListener('keydown', (e) => { if (e.key === 'Enter') confirmEanAssociation(); });
    $('#ean-assoc-modal-backdrop').addEventListener('click', (e) => {
      if (e.target === $('#ean-assoc-modal-backdrop')) skipEanAssociation();
    });

    // --- Profile ---
    $('#profile-logout-icon-btn').addEventListener('click', logout);
    $('#profile-sync-card').addEventListener('click', openServerModal);

    // --- Server config modal ---
    // Le noteur doit activer la connexion lui-même (jamais cochée
    // automatiquement — voir WebzaConfig.isServerEnabled, défaut false).
    // Tout le reste (test, synchro des opérations en attente) est
    // automatique en arrière-plan — voir startTimers() — sans bouton manuel.
    $('#server-enabled-toggle').addEventListener('click', () => {
      const nowEnabled = !WebzaConfig.isServerEnabled();
      WebzaConfig.setServerEnabled(nowEnabled);
      renderServerEnabledToggle();
      if (nowEnabled) {
        testServerConnection().then(() => syncPendingOperations());
      } else {
        state.isServerConnected = false;
        state.lastSyncMessage = 'Connexion au serveur désactivée manuellement.';
        renderServerBadge();
        renderServerModalStatus();
      }
    });
    $('#server-modal-close-btn').addEventListener('click', closeServerModal);
  ...
  function startTimers() {
    ...
    setInterval(async () => {
      updatePendingCount();
      if (!WebzaConfig.isServerEnabled()) return; // coupée volontairement, rien à tenter
      const pending = OperationRepository.getPendingSync();
      let res;
      if (pending.length > 0) {
        res = await SyncManager.sendOperations(pending);
        if (res.success) {
          pending.forEach((op) => OperationRepository.updateOperation({ ...op, syncStatus: 'SYNCED' }));
          updatePendingCount();
        }
      } else {
        // Rien à envoyer, mais on vérifie quand même la liaison pour garder
        // le statut affiché sur l'écran Profil à jour en continu.
        res = await SyncManager.testConnection();
      }
      if (res.success) {
        state.isServerConnected = true;
        flushPendingEanAssociations();
      } else if (!res.disabled) {
        // Échec réel (pas une désactivation volontaire détectée entretemps) :
        // on coupe nous-même l'interrupteur, sans action du noteur.
        state.isServerConnected = false;
        WebzaConfig.setServerEnabled(false);
        renderServerEnabledToggle();
      }
  ...
  function init() {
    ...
    const eanRefList = $('#ean-assoc-ref-list');
    if (eanRefList) {
      eanRefList.innerHTML = PRODUCT_REFERENCES.map((r) => `<option value="${r}"></option>`).join('');
    }
