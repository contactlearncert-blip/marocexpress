...
(() => {
  ...
  function renderPin() {
    ...
    $$('.key').forEach((k) => {
      k.disabled = state.lockoutSeconds > 0;
    });
  }
  ...
  async function verifyPin(pin) {
    ...
    const matched = OPERATORS.find((o) => o.codeHash === hash);
    if (matched) {
      ...
      state.pinError = null;
      loginSuccess();
      // Vérification locale = retour visuel instantané (fonctionne
      // hors-ligne, terrain). L'AUTORISATION réelle pour l'API reste la
      // session serveur ouverte ici : tant qu'elle n'est pas établie
      // (hors-ligne), les écritures restent en file locale (voir sync.js
      // ensureSession()/sendOperations()) et ne sont jamais considérées
      // comme définitivement enregistrées côté backend.
      SyncManager.login(pin).then((res) => {
        if (!res.success && !res.offline) {
          // Le serveur refuse ce code alors que la copie locale (roster
          // en cache) le reconnaissait : rejoue une synchro impossible
          // tant que la session n'est pas rétablie ; on informe l'opérateur
          // sans le déconnecter de l'UI locale (usage terrain hors-ligne).
  ...
  function confirmCancelOp() {
    ...
    OperationRepository.deleteOperation(opToCancel);
    showToast(`Opération ${opToCancel.reference} annulée / supprimée`);
    closeCancelModal();
    updatePendingCount();
  ...
  function wireEvents() {
    ...
    $('#loc-mode-defined').addEventListener('click', () => {
      ...
      if (!state.formState.location || /^N[012]$/.test(state.formState.location)) {
        state.formState.location = LOCATIONS[0] || 'N0_ZC_E1';
      }
      renderForm();
    ...
    $('#success-cancel-btn').addEventListener('click', () => {
      if (!state.lastOperation) return;
      OperationRepository.deleteOperation(state.lastOperation);
      showToast(`Opération ${state.lastOperation.reference} annulée / supprimée`);
      updatePendingCount();
      goHome();
    });
