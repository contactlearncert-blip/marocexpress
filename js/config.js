...
const WebzaConfig = {
  ...
  // --- Connexion au serveur (ON/OFF) ------------------------------------
  // Interrupteur manuel, indépendant de la connectivité réseau réelle du
  // téléphone : permet à l'opérateur de couper volontairement toute
  // tentative de communication avec le serveur (voir sync.js
  // _fetchWithTimeout, seul point de passage de toutes les requêtes).
  // Désactivé par défaut : jamais coché automatiquement, c'est le noteur
  // qui l'active lui-même. En revanche, si une tentative de connexion
  // échoue réellement pendant qu'il est activé, il se désactive tout seul
  // (voir testServerConnection/syncPendingOperations, app.js) — sans avoir
  // besoin de rouvrir cet écran. L'URL elle-même n'est en revanche plus
  // jamais affichée ni modifiable depuis l'interface (écran Réglages > Serveur).
  ENABLED_STORAGE_KEY: 'webza_server_enabled',
