...
const SyncManager = {
  ...
  async _fetchWithTimeout(url, options, timeoutMs) {
    if (!WebzaConfig.isServerEnabled()) {
      // Connexion au serveur désactivée manuellement (interrupteur ON/OFF,
      // écran Réglages > Serveur) : toute requête échoue immédiatement,
      // exactement comme si le réseau était coupé — chaque appelant traite
      // déjà ce cas (file d'attente locale, message hors-ligne...).
  ...
  async testConnection(customUrl) {
    ...
    try {
      ...
      if (e.message === 'server_disabled') {
        return { success: false, disabled: true, message: 'Connexion au serveur désactivée.' };
      }
    ...
    try {
      const payload = {
        operations: operations.map((op) => ({
          ...
          hasDefect: !!op.hasDefect,
          containerSessionId: op.containerSessionId ?? null,
        })),
      };
      const res = await this._fetchWithTimeout(
        `${targetUrl}/api/sync`,
  ...
  /**
   * Catalogue EAN -> référence appris côté serveur (voir
   * routers/ean_codes.py) : complète le catalogue livré avec l'app
   * (EAN_TO_REFERENCE, data.js), qui reste la base de départ hors-ligne.
   * Retourne {} en cas d'échec (hors-ligne, etc.) — jamais d'exception.
   */
  async fetchEanCodes() {
    const hasSession = await this.ensureSession();
  ...
  /**
   * Enregistre l'association d'un code-barres scanné mais non reconnu à
   * une référence, saisie par le noteur sur le moment — voir le prompt
   * dédié dans app.js (openBarcodeScanner / le champ Référence). Partagé
   * avec tous les téléphones dès leur prochaine synchronisation.
   */
  async registerEanCode(ean, reference) {
    const hasSession = await this.ensureSession();
    if (!hasSession) {
    ...
    try {
      const res = await this._fetchWithTimeout(
        `${targetUrl}/api/ean-codes`,
        {
          ...
          credentials: 'include',
          headers: { 'Content-Type': 'application/json; charset=utf-8' },
          body: JSON.stringify({ ean, reference }),
        },
        5000
      );
      ...
      return { success: false, reason: e.message === 'server_disabled' ? 'disabled' : 'offline' };
    }
  },

  /**
   * Auto-annulation « dans les 24h » (voir js/app.js — WITHIN_SELF_CANCEL_MS) :
   * supprime réellement l'opération côté serveur (stock inclus), et non
   * plus seulement en local. Passé ce délai, l'app ne doit plus appeler
   * cette méthode : l'annulation reste alors purement locale au téléphone.
   * Retourne toujours un objet {success, reason?} — jamais d'exception —
   * pour que l'appelant puisse décider comment informer le noteur.
   */
  async selfCancelOperation(serverUuid) {
    const hasSession = await this.ensureSession();
    if (!hasSession) {
      return { success: false, reason: 'offline' };
    }
    ...
    try {
      const res = await this._fetchWithTimeout(
        `${targetUrl}/api/operations/mobile/${encodeURIComponent(serverUuid)}`,
        { method: 'DELETE', credentials: 'include' },
        8000
      );
      if (res.ok) return { success: true };
      if (res.status === 404) return { success: true }; // déjà supprimée côté serveur : rien à faire
      if (res.status === 409) return { success: false, reason: 'too_old' };
      if (res.status === 403) return { success: false, reason: 'not_own' };
  ...
  /**
   * Pendant du selfCancelOperation ci-dessus, pour l'ÉDITION (voir
   * js/app.js — cancelOperation / editOperation, même fenêtre de 24h) :
   * répercute une correction sur le serveur (stock recalculé), et non
   * plus seulement en local. `changes` utilise les noms de champs
   * attendus par le backend (snake_case pour truck_type).
   */
  async selfEditOperation(serverUuid, changes) {
    const hasSession = await this.ensureSession();
    if (!hasSession) {
      return { success: false, reason: 'offline' };
    }
  ...
  /**
   * Prise en charge d'un container avant de saisir des lignes de réception
   * dessus (voir js/app.js — openContainerModal). L'arbitrage anti-doublon
   * entre deux noteurs se fait ICI, côté serveur — jamais côté téléphone.
   * Retourne {success:true, session} ou {success:false, reason, holderName?}.
   */
  async claimContainer(code) {
    const hasSession = await this.ensureSession();
    if (!hasSession) {
      return { success: false, reason: 'offline' };
    }
    ...
    try {
      const res = await this._fetchWithTimeout(
        `${targetUrl}/api/containers/claim`,
        {
          method: 'POST',
          credentials: 'include',
          headers: { 'Content-Type': 'application/json; charset=utf-8' },
      ...
      if (res.status === 409) {
        ...
        return { success: false, reason: 'busy', holderName: detail.holderName };
      }
      return { success: false, reason: `http_${res.status}` };
    } catch (e) {
      return { success: false, reason: 'offline' };
  ...
  /** Clôture volontaire ('Terminer ce container' — voir js/app.js). */
  async closeContainer(sessionId) {
    const hasSession = await this.ensureSession();
    if (!hasSession) {
      return { success: false, reason: 'offline' };
    }
    ...
    try {
      const res = await this._fetchWithTimeout(
        `${targetUrl}/api/containers/${sessionId}/close`,
        { method: 'POST', credentials: 'include' },
        8000
      );
      if (res.ok) return { success: true };
