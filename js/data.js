...
// Emplacements de racks structurels (Niveaux N0, N1, N2 avec Zones A, B, C, D, E)
function buildLocations() {
  const zoneCounts = {
    0: { A: 3, B: 9, C: 12, E: 2 },
    1: { A: 4, B: 10, C: 18, D: 6, E: 3 },
    2: { A: 4, B: 10, C: 18, D: 6, E: 3 },
  };
  const list = [];
  for (const level of Object.keys(zoneCounts)) {
    const zones = zoneCounts[level];
    for (const zone of Object.keys(zones)) {
      const count = zones[zone];
      for (let i = 1; i <= count; i++) {
        list.push(`N${level}_Z${zone}_E${i}`);
      }
    }
  }
  return list;
